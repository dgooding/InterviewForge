...
> = {
  clarity: {
    ...
    psychFrame:
      "Cognitive fluency — listeners treat clear speech as more credible and competent.",
  },
  relevance: {
    label: "Relevance & focus",
    psychFrame:
      "Goal-directed attention — staying on the question signals executive control under evaluation.",
  },
  structure: {
    label: "Narrative structure",
    psychFrame:
      "Narrative psychology / STAR — structured stories are easier to encode and trust.",
  },
  technicalAccuracy: {
    label: "Technical depth",
    psychFrame:
      "Domain expertise & elaborative knowledge — accurate models under questioning.",
  },
  confidence: {
    label: "Presence & confidence",
    psychFrame:
      "Self-presentation & self-efficacy — calm delivery without empty bravado.",
  },
};
...
function dimInterpretation(key: Exclude<DimensionKey, "overall">, score: number): {
  ...
  if (score <= 0) {
    return {
      interpretation: "Not enough samples yet to profile this dimension.",
      coachCue: "Complete a few scored answers to unlock this signal.",
    };
  }
  if (key === "clarity") {
    if (score >= 8)
      return {
        interpretation:
          "Your wording is easy to follow under pressure — high processing fluency for interviewers.",
        coachCue: "Keep using short sentences; add one concrete metric per story.",
      };
    if (score >= 6)
      return {
        interpretation:
          "Mostly clear, with occasional dense or meandering stretches that tax the listener.",
        coachCue: "Practice the 3-sentence open: context → action → result.",
      };
    return {
      interpretation:
        "Listeners may struggle to track your point — common under evaluation anxiety.",
  ...
  if (key === "structure") {
    if (score >= 8)
      return {
        interpretation:
          "Strong narrative scaffolding (STAR-like). Interviewers can store and retell your story.",
        coachCue: "Vary emotional beat without losing structure — still end with result.",
      };
    if (score >= 6)
      return {
        interpretation:
          "Structure is present but uneven — sometimes Situation runs long or Result is thin.",
        coachCue: "Force: 20% context, 50% actions, 30% result + learning.",
      };
    return {
      interpretation:
        "Stories risk sounding chronological dumps rather than causal narratives.",
      coachCue: "Write STAR labels on scratch paper before you speak.",
    };
  }
  if (key === "confidence") {
    if (score >= 8)
      return {
        interpretation:
          "High presence. Ensure confidence tracks substance (see calibration below).",
        coachCue: "Keep warmth; avoid absolute claims you can't defend.",
      };
    if (score >= 6)
      return {
        interpretation:
          "Adequate presence with room for calmer pacing and cleaner closes.",
        coachCue: "End answers with a firm result sentence, not a fade-out.",
      };
    return {
      interpretation:
        "Low presence can be read as low competence even when content is solid (self-presentation bias).",
  ...
  if (key === "technicalAccuracy") {
    if (score >= 8)
      return {
        interpretation:
          "Domain models hold up under probing — strong elaborative expertise signal.",
        coachCue: "Add trade-offs and failure modes to sound senior.",
      };
    if (score >= 6)
      return {
        interpretation:
          "Solid foundation with gaps under depth questions.",
        coachCue: "For each concept, prep: definition → example → trade-off.",
      };
    return {
      interpretation:
        "Technical answers need tighter accuracy and fewer vague claims.",
      coachCue: "Drill fundamentals aloud; stop when unsure and reason out loud.",
    };
  }
  // relevance
  if (score >= 8)
    return {
      interpretation:
        "You stay tightly coupled to the question — strong goal-directed attention.",
      coachCue: "Keep echoing the question's verb in your first line.",
    };
  if (score >= 6)
    return {
      interpretation:
        "Mostly on-target with occasional digressions under stress.",
      coachCue: "If you drift, say 'Back to your question…' and re-anchor.",
    };
  return {
    interpretation:
      "Answers often leave the interviewer's question behind — costly under limited time.",
...
function computeTrajectory(scores: number[]): Trajectory {
  if (scores.length < 2) {
    return {
      ...
      insight:
        "Self-efficacy grows from repeated mastery experiences. You need a few more sessions for a trend.",
      selfEfficacyNote:
        "Bandura: early wins matter — prioritize winnable drills, then stretch.",
    };
  }
  ...
  if (sd >= 1.8 && Math.abs(delta) < 0.6) {
    return {
      ...
      label: "High variance",
      insight:
        "Performance swings suggest state anxiety or inconsistent prep — not fixed ability.",
      selfEfficacyNote:
        "Stabilize with a pre-interview ritual (breath + 30s plan) to reduce noise.",
    };
  }
  if (delta >= 0.5) {
    return {
      ...
      insight: `Recent sessions average ${delta} points higher — mastery experiences are accumulating.`,
      selfEfficacyNote:
        "Protect the streak of wins; layer harder modes only after two solid sessions.",
    };
  }
  if (delta <= -0.5) {
    return {
      ...
      label: "Dip detected",
      insight:
        "Recent scores are softer. Often fatigue, harder modes, or cramming — not permanent decline.",
      selfEfficacyNote:
        "Return to a medium drill you can nail, then re-challenge. Recovery rebuilds efficacy.",
    };
  }
  return {
    direction: "stable",
    ...
    insight:
      "Scores are consistent. Plateaus are normal before the next deliberate-practice jump.",
    selfEfficacyNote:
      "Break the plateau with one focused weak dimension (e.g. structure-only drills).",
  };
}
...
function computeCadence(sessions: InterviewSession[]): PracticeCadence {
  if (!sessions.length) {
    return {
      ...
      spacingLabel: "none",
      spacingInsight:
        "Spacing effect: distributed practice beats cramming for durable interview skill.",
    };
  }
  ...
  if (daysSinceLastPractice >= 14) {
    spacingLabel = "lapsed";
    spacingInsight =
      "14+ days idle — expect mild skill fade (forgetting curve). A short warm-up session restores access quickly.";
  } else if (averageGapDays != null && averageGapDays < 1.2 && days.length >= 3) {
    spacingLabel = "crammed";
    spacingInsight =
      "Sessions cluster tightly. Cramming raises short-term scores but weaker long-term retention — leave 1–2 days between deep practices.";
  } else if (averageGapDays != null && averageGapDays > 7) {
    spacingLabel = "sparse";
    spacingInsight =
      "Long gaps between sessions. Aim for 2–4 focused sessions/week for habit formation and spacing benefits.";
  } else {
    spacingLabel = "healthy";
    spacingInsight =
      "Practice rhythm looks sustainable — consistent exposure builds automaticity under stress.";
  }
...
function computeCalibration(answers: InterviewAnswer[]): ConfidenceCalibration {
  if (!answers.length) {
    return {
      ...
      gap: 0,
      label: "insufficient",
      insight: "Need scored answers to calibrate confidence vs substance.",
  ...
  if (gap >= 1.2) {
    return {
      ...
      label: "overconfident",
      insight:
        "Presence outruns substance. Interviewers often probe harder when delivery is slick but thin.",
      coachCue:
  ...
  if (gap <= -1.2) {
    return {
      ...
      insight:
        "Content is stronger than delivery. Self-presentation bias means you may be underrated live.",
      coachCue:
        "Practice power closes and 10% slower pace; your substance already carries weight.",
    };
  }
  return {
    confidence,
    ...
    label: "calibrated",
    insight:
      "Confidence roughly matches substance — healthy self-presentation without empty bravado.",
    coachCue: "Maintain calibration; record one session weekly to keep the match.",
  };
}
...
function growthFromScores(scores: number[]): { score: number; insight: string } {
  if (scores.length < 3) {
    return {
      score: 5,
      insight:
        "Growth mindset shows in bounce-back after dips. Need more sessions to measure recovery.",
    };
  }
  let recoveries = 0;
  let opportunities = 0;
  ...
  if (opportunities === 0) {
    return {
      score: 7,
      insight:
        "Few sharp dips — stability is good. Growth shows when you seek harder modes and still recover.",
    };
  }
  ...
  return {
    ...
    insight:
      ratio >= 0.6
        ? "You tend to rebound after weaker sessions — a growth-oriented pattern (effort → recovery)."
        : "After dips, scores stay soft. Treat bad sessions as data; schedule a deliberate recovery drill within 48h.",
  };
}
...
function deliberatePractice(
  ...
  if (!sessions.length) {
    return {
      score: 0,
      insight: "Deliberate practice needs goals, feedback, and repetition.",
    };
  }
  ...
  return {
    ...
    insight:
      score >= 7
        ...
        : "Shift from random reps to deliberate practice: one weak skill per session + review feedback aloud.",
  };
}
...
function buildRecommendations(
    ...
    return [
      {
        ...
        why: "Mastery experiences build self-efficacy fastest in structured story formats.",
      ...
      },
      {
        ...
        why: "Role clarity focuses retrieval practice on relevant schemas.",
        href: "/roles",
        priority: "medium",
  ...
  if (profile.cadence.spacingLabel === "crammed") {
    recs.push({
      ...
      why: "Let consolidation happen; review one PDF report instead of another full mock today.",
      href: "/history",
      priority: "medium",
  ...
  if (profile.diversityScore < 4 && sessions.length >= 3) {
    recs.push({
      ...
      why: "Skill variety improves transfer and reduces brittle, mode-specific performance.",
  ...
  if (strongest && strongest.score >= 7.5) {
    recs.push({
      ...
      why: "Lead interviews with signature strengths while you remediate weaker dimensions (broaden-and-build).",
      href: "/questions",
      priority: "low",
  ...
  recs.push({
    title: "Question bank — deliberate micro-reps",
...
export function buildPsychProfile(
  ...
  const diversityInsight =
    diversityScore >= 6
      ? "Good mode variety — flexible retrieval under different evaluation formats."
      : diversityScore >= 3
        ? "Some variety. Adding company or system-design modes improves transfer."
        : "Low variety. Performing well in one mode may not transfer — cross-train.";
  ...
  let timedInsight =
    "Not enough timed vs untimed data yet to map arousal–performance (Yerkes–Dodson).";
  if (timedAvg != null && untimedAvg != null) {
    const d = Math.round((untimedAvg - timedAvg) * 10) / 10;
    if (d >= 0.8) {
      timedInsight = `Untimed scores run ~${d} higher — evaluation arousal may be costing you under the clock. Practice timed drills to desensitize.`;
    } else if (d <= -0.8) {
      timedInsight = `Timed scores are higher — light time pressure may be focusing attention (optimal arousal). Keep light timers.`;
    } else {
      timedInsight =
        "Timed and untimed performance are similar — solid regulation under clock pressure.";
    }
  } else if (timedAvg != null) {
    timedInsight =
      "Mostly timed practice — good exposure to evaluation stress. Add one untimed deep drill weekly for reflection.";
  }
  ...
  const narrativeInsight =
    structureScore >= 8
      ? "Narrative maturity is high — stories are causally coherent and memorable."
      : structureScore >= 6
        ? "Narratives work but Results/Learning often underweight. Interviewers remember endings."
        : "Build STAR muscle memory; unstructured stories overload interviewer working memory.";
  ...
  let depthInsight = "Answer length will appear after a few responses.";
  if (answers.length) {
    if (avgWords < 40) {
      depthInsight =
        "Answers are very short — may under-share evidence. Aim ~90–150 words for behavioral stories.";
    } else if (avgWords > 280) {
      depthInsight =
        "Answers run long — risk of cognitive overload for the listener. Tighten Actions; cut backstory.";
    } else {
      depthInsight =
  ...
  else if (readinessIndex >= 45) readinessLabel = "Developing proficiency";
  ...
  const readinessInsight =
    sessions.length === 0
      ? "Readiness compounds with deliberate reps. Start with one behavioral session to set your baseline."
      : `Composite of score level, trajectory, practice rhythm, confidence calibration, and deliberate-practice habits. Streak: ${streak} day${streak === 1 ? "" : "s"}.`;
...
function buildNarrative(p: PsychProfile): string[] {
  ...
  lines.push(
    `**Confidence calibration:** ${p.calibration.label}. ${p.calibration.insight}`
  );
  lines.push(`**Narrative skill:** ${p.narrativeInsight}`);
  lines.push(`**Deliberate practice:** ${p.deliberateInsight}`);
  if (p.topStrengths[0]) {
    lines.push(
      `**Signature strength theme:** “${p.topStrengths[0].text}” (recurring in feedback).`
    );
  }
  if (p.topImprovements[0]) {
    lines.push(
      `**Primary growth edge:** “${p.topImprovements[0].text}” — treat as this week’s deliberate focus.`
    );
  }
  lines.push(`**Mode transfer:** ${p.diversityInsight}`);
  lines.push(`**Arousal & timing:** ${p.timedVsUntimed.insight}`);
  return lines;
}
